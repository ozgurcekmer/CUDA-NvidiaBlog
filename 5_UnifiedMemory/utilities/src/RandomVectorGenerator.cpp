...
template <typename T>
void RandomVectorGenerator<T>::randomVector(Vector::managedVector<T>& v) const
{
    std::random_device rd;
    std::mt19937 randEng(rd());
...
template void RandomVectorGenerator<float>::randomVector(Vector::managedVector<float>& v) const;
template void RandomVectorGenerator<double>::randomVector(Vector::managedVector<double>& v) const;